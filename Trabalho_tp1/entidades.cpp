...
Quarto::Quarto(const Numero& num, const Capacidade& cap, const Dinheiro& v, const Ramal& ram)
    : numero(num), descricao(), capacidade(cap), valor(v), ramal(ram) {}

void Quarto::setNumero(const Numero& n)      { numero    = n; }
void Quarto::setDescricao(const Nome& d)     { descricao = d; }
...
Nome      Quarto::getDescricao()  const { return descricao; }
...
void Reserva::exibir_reserva() {
    ...
    if (quarto) {
        ...
        cout << "Diária: " << quarto->getValor().getValor() << endl;
        cout << "Dias: " << dias << endl;
...
void Reserva::exibirTudo() const {
    ...
    if (quarto) {
        ...
        ostringstream oss;
        oss << fixed << setprecision(2) << total;
        cout << "Diária: " << quarto->getValor().getValor() << "\n";
        cout << "Dias: " << dias << "\n";
